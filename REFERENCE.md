...
| `hm_lesson_order` | Teacher-set custom display order for a course's lessons page, doc ID = course key (`radio`\|`live`\|`yearbook`\|`sports`\|`indepth`\|`intro`): `{order: [lessonId,...]}` covering both built-in (data.js) and Canva lesson IDs in one flat list (no per-unit grouping — see `getCourseLessonList()`/`moveLessonItem()` in script.js). IDs not in the array fall back to default order (data.js order, then Canva lessons by creation time) appended at the end. Teacher-only ▲/▼ buttons on each lesson item write this doc |
